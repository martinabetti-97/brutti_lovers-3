...
```
As we can observe, the combination that generates the lowest RMSE is the case in which we reduce the 171 features into 2 components. We will also tune the randomForest parameters in a similar fashion, by picking those parameters that give the lowest RMSE on validation:

```{r, grid search, echo=FALSE, fig.width=10, fig.height=8, fig.align='center'}

p = 39
simulations = 5
ntree_grid = c(100, 300, 500, 700, 900)
mtry_grid = c(p/7, p/5, p/3, p/2, p-2)
sim_matrix = matrix(NA, nrow=nrow(expand.grid(ntree_grid, mtry_grid)), 
                    ncol=simulations)
sim_matrix = cbind(expand.grid(ntree_grid, mtry_grid), sim_matrix)
names(sim_matrix) <- c('ntree', 'mtry', 's1', 's2')
...
for (sim in 1:simulations) {
  ...
  ind <- sample(c(TRUE, FALSE), n, replace=TRUE, prob=c(0.7, 0.3))
    ...
    sim_matrix[i, sim+2] = rmse_val
  }
}

mean = rowMeans(sim_matrix[,3:ncol(sim_matrix)])
sim_matrix = cbind(sim_matrix, mean)
sim_matrix
```
...
best_mtry = as.numeric(sim_matrix[which.max(sim_matrix$mean),]['mtry'])
best_ntree = as.numeric(sim_matrix[which.max(sim_matrix$mean),]['ntree'])

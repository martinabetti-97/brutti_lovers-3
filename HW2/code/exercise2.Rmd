...
gradient_descent <- function(X, Y, G, alpha=0.5, max_iter=100, tol=0.001, beta=NULL, plot=FALSE) {
  ###
  ...
  for(i in 1:d) {
    for(j in 1:d) {
      X.T_X[i, j] <- t(X[,i]) %*% X[,j] * 2 / n # precompute X.T * X
    }
...
```{r}
n <- length(y_tr)
idxs <- sample(seq(1, n), size = 10000, replace = FALSE)
```

Encode the labels and compute the parameters of the trained model:

```{r}
y_tr_encoded <- encode(y_tr[idxs])

beta <- gradient_descent(X_tr[idxs,], y_tr_encoded, gradient_perceptron, alpha=0.5, max_iter=200, plot=TRUE)$param
```

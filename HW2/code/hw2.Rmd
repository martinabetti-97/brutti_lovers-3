...
For the stochastic implementation of the gradient descent at each iteration we consider a single data point of the original training set, which is drawn at random. Since we only consider a single point, we don't need to compute the gradient for the whole matrix since for each iteration the gradient is defined as the product $\mathbb{X}^T \mathbb{X}$ where $\mathbb{X}$ is a vector representing all the features of the point. The $\mathbf{Y}$ is instead represented by a single integer 0 or 1 which is the true value of Y for the data point. 

This approach is based on the assumption that the noise of the sampling will cancel out, however we expect it to have some effects on the performance, especially on the ER. We also expect this version of the algorithm to need more iterations to converge since the information gained at each iteration is less compared to the one in batch implementation, let's check.

```{r}
# Define the gradient descent function
gradient_descent_stochastic <- function(X, Y, alpha=0.5, max_iter=100, tol=0.000001, beta=NULL, plot=FALSE) {
  ...
  for(i in 1:max_iter) {
    ...
    X.T_X <- t(X[idx,]) %*% X[idx,] * 2 # n = 1 ?
    X.T_Y <- t(X[idx,]) %*% X[idx,] * Y[idx] * 2
    ...
    # Compute and store the ER value of the i-th iteration
    emp_risk_hist[i+1] -> cbind(1,X[idx,]) %*% beta - Y[idx]
...
result_st <- gradient_descent_stochastic(X_tr, y_tr_encoded, alpha=0.02, max_iter = 3000,  plot=T)

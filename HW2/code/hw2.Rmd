...
For the stochastic implementation of the gradient descent at each iteration we select only N data points from the original training set, which are drawn randomly and without replacement. Since the set of data points considered for each iteration are different the gradient changes at each iteration, implying that we cannot pre-compute it. Therefore, even though this algorithm design is faster in principle, we expect it to perform worst than the previous implementation which only computed the gradient once. However, it is worth mentioning that this is true in this specific case because the feature matrix we are dealing with is very sparse and so we were able to optimize the memory load and running time for the computation. Whenever this does not hold true, the stochastic version of the algorithm may be the best approach to avoid loading the data all at once. 

```{r}
# Define the gradient descent function
gradient_descent_stochastic <- function(X, Y, G, alpha=0.5, max_iter=100, N= 1000, tol=0.000001, beta=NULL, plot=FALSE) {
  ...
  for(i in 1:max_iter) {
    ...
    idxs <- sample(seq(1, n), size = 2000, replace = FALSE)
    
    X.T_X <- crossprod(cbind(1,X[idxs,])) * (2 / n)
    X.T_Y <- crossprod(cbind(1,X[idxs,]), Y[idxs]) * (2 /n)

    grad <- G(X.T_X, X.T_Y, beta)
    
    # Update beta
    #alpha_opt <- alpha/(i%%100 +1)
    beta <- beta - alpha * grad # update rule
...
By testing the algorithm with a subset of 100 data points, we can easily verify that for the same value of alpha, the previous implementation of the GD was much faster than this one. In terms of accuracy we can observe that selecting a subset of data points did decrease the performances by a little, however if the number of sample drawn is large enough the difference is not strongly significant. 

```{r}
result_st_100 <- gradient_descent_stochastic(X_tr, y_tr_encoded, gradient_linear, alpha=0.4, max_iter = 3000, N = 100,  plot=T)

pred_st_100<- linear_classifier(X_te, result_st_100$param)

accuracy_st_100 <- sum(pred_st_100 == y_te_encoded) / length(y_te_encoded)

cat(paste("Accuracy of the model:", accuracy_st_100))
```
...
However, to provide a better approximation error, it may be convenient to use a non-linear approximation of the form $S_{\Delta}(f, B)$ to the Doppler function instead one of the form $S_m(f, B)$. 
...
Since in the non-linear case this approximation applies only to the case of an orthonormal system $B$ of a Hilbert space, we will consider the cosine basis. Below we define the function describing the cosine basis and the Doppler function.
...
In the linear case we approximate our function by picking the the first $m$ Fourier transforms. In the non-linear case we pick $m$ indices corresponding to the greater Fourier transforms in absolute values.
...
By looking at the the table above we can actually verify that, for any $j$, the non-linear approximation is closer to the true Doppler function than its linear counterpart. 

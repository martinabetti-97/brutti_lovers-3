...
fhat_plot <- function(q, train) {
  ...
  plot( y ~ x , train, lwd = 2, col = rgb(.1, .1, .1, .6))
  lines(fhat ~ x, train, lwd = 4, col = rgb(0.941, 0.502, 0.502, .8))
}
```
...
plot(cv, lwd = 3, type = "b", col = cols[1], xlab = "Model", ylab = "CV")
...
Even though the values of the three metrics taken into account apparently disagree on the best model to pick, the value of the `5-fold CV` on $q=5$ pushes us to select this value as the optimal for $q$. Eventually, the best model on the data is $\hat{f}_{spline}(x; d=3, q=5)$.
...
In the end, we compare the best spline model $\hat{f}_{spline}(x; d=3, q=5)$ with the polynomial regression model $\hat{f}_{poly}(x; d=3)$ of degree 3 and the true data generating function.

```{r final_plot, fig.width=10, fig.height=6, echo=FALSE}
cols = c(rgb(0.941, 0.502, 0.502, .8), rgb(0.345, 0.502, 0.234, .6), rgb(.1, .1, .1, .4))
fhat_plot(q = 3, train)
lines(poly_func(d = 3) ~ x, train, lwd = 4, col = cols[2])
lines(train$x, ftrue, lwd = 4, lty = 2, col = cols[3])
legend("bottomright", legend = c("degree-3 regression spline", 
                                 "degree-3 polynomial regression", 
                                 "true function"), 
       col = cols, lty = c(1,1,2), lwd = 3, cex = .8)
```
...
`r options(scipen=10); mean_squared_error(test$y, fhat_func(3, predict_sample=test))`, whereas the one of the polynomial regressor is $MSE_{Te}(\hat{f}_{poly})=$ 
